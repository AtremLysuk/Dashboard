...
type TOrder = Pick<Order, "id" | "createdAt" | "status" | "customerName"> & {
  items: TOrderItem[];
  client: TClient;
};
...
export default function OrderCard({ order, className }: Props) {
  ...
  const titleId = `order=${id}-title`;
  const meta = ORDER_STATUS_UI[status as OrderStatus]; // Приводим к типу

  const quantity = items.reduce((accum, product) => {
    accum += product.quantity;
    return accum;
  }, 0);

  const totalOrderPrice = items.reduce((sum, product) => {
    sum += product.price * product.quantity;
    return sum;
  }, 0);

  const isFinalStatus = status === "COMPLETED" || status === "REJECTED" || status === "CANCELLED";
